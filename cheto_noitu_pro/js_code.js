...
// 2. Gọi API với nội dung vừa lấy được
fetch(`/answer?word=${encodeURIComponent(spanContent)}`)
  .then(response => response.json())
  .then(data => {
    // 3. Lấy nội dung trong tail từ JSON được trả về
    const tailContent = data.nextWord.tail;

    // 4. Điền nội dung từ tail vào hộp input có id là "text"
    const inputElement = document.getElementById('text');
    inputElement.value = tailContent;

    // 5. Tạo một sự kiện nhấn Enter và tự động submit form
    const form = inputElement.form; // lấy form chứa hộp input
    if (form) {
      form.submit();
    } else {
      // Nếu không có form, có thể tự tạo sự kiện Enter và dispatch nó
