...
async function generateNickname() {
    const adjectives = ["Sieu", "Thanh", "Ba", "Huyen", "Sat", "Chien", "Lanh", "Nong", "Manh", "Doc", "Lem", "Banh", "Quay", "ThuLinh"];
    const nouns = ["Ho", "Rong", "Soi", "DaiBang", "Cao", "BoCap", "Nhen", "KenKen", "Voi", "SuTu", "Gau", "Bao", "Chon", "ChimUng"];

    const adjNgauNhien = adjectives[Math.floor(Math.random() * adjectives.length)];
    const nounNgauNhien = nouns[Math.floor(Math.random() * nouns.length)];
    const number = Math.floor(Math.random() * 10000);  // Tạo số ngẫu nhiên từ 0 đến 9999

    const nickname = `${adjNgauNhien}${nounNgauNhien}${number}`;
    // sinh tên thành công
    
    Cookies.set("name2", nickname);
    // Hiển thị modal và đổi nickname
    // $("#changeNameModal").modal();
    // document.getElementById('username').value = nickname;
    // await sleep(1000);  // Nghỉ 1 giây
    
    // // Auto click nút Lưu
    // document.querySelector('.btn.btn-primary').click();
    
    // console.log("Đã thay đổi nickname thành:", nickname);
}
